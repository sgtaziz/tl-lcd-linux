...
use crate::config::{SensorDescriptor, SensorRange, SensorSourceConfig};
...
impl SensorAsset {
    pub fn new(descriptor: &SensorDescriptor, orientation: f32) -> Result<Arc<Self>, MediaError> {
        ...
        // Ensure there's a fallback range with max: None
        ...
        let max_radius = (LCD_WIDTH.min(LCD_HEIGHT) as f32 / 2.0) - 6.0;
    ...
    pub fn render_frame(&self) -> Result<Vec<u8>, MediaError> {
        ...
        let mut image = ImageBuffer::from_pixel(LCD_WIDTH, LCD_HEIGHT, Rgb(self.background_color));
        ...
        if let Some(font) = &self.font {
            draw_sensor_text_ttf(&mut image, text_params, font);
        } else {
            draw_sensor_text_fallback(&mut image, text_params);
        }
    ...
    pub fn blank_frame(&self) -> Vec<u8> {
        let image = ImageBuffer::from_pixel(LCD_WIDTH, LCD_HEIGHT, Rgb(self.background_color));
        let oriented = apply_orientation(image, self.orientation);
        encode_jpeg(&oriented).unwrap_or_default()
    }
...
fn draw_gauge(image: &mut RgbImage, params: GaugeParams) {
    let GaugeParams {
        value,
        gauge_color,
        ring_color,
    ...
    let cx = (LCD_WIDTH as f32 - 1.0) / 2.0;
    let cy = (LCD_HEIGHT as f32 - 1.0) / 2.0;
    let max_radius = (LCD_WIDTH.min(LCD_HEIGHT) as f32 / 2.0) - 4.0;
        ...
        for x in 0..LCD_WIDTH {
            ...
            if dist <= outer && dist >= inner {
                ...
                if diff <= sweep {
                    ...
                    // Apply corner radius to both START and END caps
                    if corner_radius > 0.0 && diff <= fill_angle && fill_angle > 0.0 {
                        let radial_mid = (inner + outer) / 2.0;

                        // Distance from the start and end of the fill
                        ...
                        // Check if we're near either cap
                        ...
                        if near_start || near_end {
                            ...
                            // Distance from the center line of the bar
                            let offset_from_center = dist - bar_center_radius;

                            // Only apply rounding when near the edges, not in the middle of the bar
                            let near_edge =
                                offset_from_center.abs() > half_thickness - corner_radius;

                            if near_edge {
                                let (x_from_corner, arc_dist) = if near_start {
                                    // Start cap - mirror so corners face left (open end faces opposite direction)
                                    (corner_radius - arc_dist_from_start, arc_dist_from_start)
                                } else {
                                    // End cap - corners face left (away from the gauge)
                                    (corner_radius - arc_dist_from_end, arc_dist_from_end)
                                };

                                // Only process if actually within corner radius range
                                if arc_dist < corner_radius {
                                    ...
                                    if corner_dist > corner_radius {
                                        // Outside the rounded corner - use background
...
fn draw_sensor_text_ttf(image: &mut RgbImage, params: TextRenderParams, font: &Font) {
    ...
    let value_text = if decimal_places > 0 {
        format!("{:.prec$}", value, prec = decimal_places as usize)
    } else {
        format!("{:.0}", value.round())
    };

    draw_text_centered(image, &value_text, value_size, color, value_offset, font);
    draw_text_centered(image, unit, unit_size, color, unit_offset, font);
    draw_text_centered(image, label, label_size, color, label_offset, font);
}
...
fn draw_text_centered(
    ...
    let start_x = ((LCD_WIDTH as f32 - text_width) / 2.0) as i32;
    let start_y = (LCD_HEIGHT as i32 / 2) + offset_y;
...
fn draw_sensor_text_fallback(image: &mut RgbImage, params: TextRenderParams) {
    let TextRenderParams {
        label,
        unit,
        value,
    ...
    let value_scale = (value_size / 4.0).max(4.0) as u32;
    let unit_scale = (unit_size / 4.0).max(3.0) as u32;
    let label_scale = (label_size / 4.0).max(3.0) as u32;
    ...
    draw_text_center_bitmap(image, &value_text, value_scale, color, value_offset);
    draw_text_center_bitmap(image, unit, unit_scale, color, unit_offset);
    draw_text_center_bitmap(image, label, label_scale, color, label_offset);
}
...
fn draw_text_center_bitmap(
    ...
    let start_x = ((LCD_WIDTH - total_width) / 2) as i32;
    let start_y = ((LCD_HEIGHT as i32) / 2) + offset_y - ((7 * scale) as i32 / 2);

    for (i, bitmap) in glyphs.iter().enumerate() {
        let base_x = start_x + i as i32 * (glyph_width as i32 + spacing as i32);
        draw_bitmap_character(image, base_x, start_y, *bitmap, scale, color);
    }
}
